...
from app.crud import task as task_crud, user as user_crud
...
def create_task(task: task_schema.TaskCreate, db: Session = Depends(get_db), current_user=Depends(auth.get_current_user)):
    db_task = task_crud.create_task(db, task, current_user.id)
    send_task_email.delay(db_task.user_id, db_task.id)
    return db_task
...
def get_task(task_id: int, db: Session = Depends(get_db), current_user=Depends(auth.get_current_user)):
    tasks = task_crud.get_task(db, task_id, current_user.id)
